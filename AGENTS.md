...
| Arlinghaus constellation architecture | `knowledge/ARLINGHAUS_SPATIAL_SYNTHESIS.md` §7 | Deployment tiers: SPU-4-only edge node (~400 LUT synthesis/P&R estimate; functional silicon proof), SPU-13 + per-axis SPU-4 cluster (bridge frames carry Davis dissonance), whisper-networked constellation; status honesty table + next steps |

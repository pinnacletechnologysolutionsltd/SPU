...
| `bash tools/zk_pnr_sweep.sh [n_seeds] [n_parallel]` | Karatsuba A/B P&R sweep: 2 spins x 2 arms x N seeds at one commit. Synthesises once per arm and **checks** that shortcut with a determinism gate that aborts if a re-synth at another seed hashes differently |
...
| Arlinghaus constellation architecture | `knowledge/ARLINGHAUS_SPATIAL_SYNTHESIS.md` §7 | Deployment tiers: SPU-4-only edge node (835 LUT4 / 390 ALU / 336 DFF measured on GW5A-25A 2026-08-14; functional silicon proof; fit on other fabrics OPEN), SPU-13 + per-axis SPU-4 cluster (bridge frames carry Davis dissonance), whisper-networked constellation; status honesty table + next steps |

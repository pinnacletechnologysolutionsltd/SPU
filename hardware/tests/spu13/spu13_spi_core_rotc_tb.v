...
module spu13_spi_core_rotc_tb;
    ...
    initial begin
        $dumpfile("build/spu13_spi_core_rotc_tb.vcd");
        $dumpvars(0, spu13_spi_core_rotc_tb);

        #5000;
        rst_n = 1'b1;
        repeat (32) @(posedge clk);

        spi_inst_write(qldi(8'd0, 8'sd1, 8'sd2, 8'sd3, 8'sd4));
        expect_qr(4'd0, 64'h0000_0000_0000_0001,
                       64'h0000_0000_0000_0002,
                       64'h0000_0000_0000_0003,
                       64'h0000_0000_0000_0004);

        spi_inst_write(rotc(8'd6, 8'd0, 6'd0));
        expect_qr(4'd6, 64'h0000_0000_0000_0001,
                       64'h0000_0000_0000_0002,
                       64'h0000_0000_0000_0003,
                       64'h0000_0000_0000_0004);

        spi_inst_write(rotc(8'd7, 8'd0, 6'd1));
        expect_qr(4'd7, 64'h0000_0000_0000_0001,
                       64'h0000_0000_0000_0003,
                       64'h0000_0000_0000_0002,
                       64'h0000_0000_0000_0004);

        if (errors == 0)
            $display("spu13_spi_core_rotc_tb: PASS");
        else
            $display("spu13_spi_core_rotc_tb: FAIL (%0d errors)", errors);
        $finish;
    end
endmodule

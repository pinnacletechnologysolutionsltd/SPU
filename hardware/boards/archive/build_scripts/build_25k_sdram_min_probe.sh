#!/usr/bin/env bash
# build_25k_sdram_min_probe.sh - Minimal SDRAM bridge probe for Tang Primer 25K
set -e
...
echo "--- 1. Yosys Synthesis (SDRAM min probe) ---"
...
nextpnr-himbaechel --device GW5A-LV25MG121NES \
    ...
    --write build/sdram_min_probe_pnr.json \
    --freq 50

echo "--- 3. Package Bitstream ---"
gowin_pack -d GW5A-25A --sspi_as_gpio --mspi_as_gpio --cpu_as_gpio build/sdram_min_probe_pnr.json -o build/tang_primer_25k_sdram_min_probe.fs

echo ""
echo "=== SDRAM Min Probe Build Complete ==="

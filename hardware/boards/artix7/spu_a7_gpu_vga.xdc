# spu_a7_gpu_vga.xdc -- identical pin mapping to spu_a7_vga_fix.xdc, the
# MEASURED J10 order (hardware_evidence.md 3.8). Same five pins, same
# resistor DAC. Only the design behind them changes.
# MEASURED pin mapping, 2026-09-04. This is the authoritative one.
#
# Determined by probing the VGA plug with J10IDENT loaded (every J10 pin
# transmitting a distinct frequency), NOT by reading a vendor pin list:
#   VGA pin 13 (HSYNC) receives FPGA D5   (6103.0 Hz, 0.0% error)
#   VGA pin 14 (VSYNC) receives FPGA E5   ( 381.5 Hz, 0.0% error)
#
# J10's TOP row follows the LiteX order (hole 1 = D5), so holes 2/3/4 are
# G5/G7/G8 = blue/green/red. Its BOTTOM row does not -- VSYNC lands on E5,
# not G6. Neither spu_a7_vga.xdc nor spu_a7_vga_rev.xdc had both halves
# right, which is why three earlier attempts all failed to lock a monitor.
#
...
set_property PACKAGE_PIN G8 [get_ports vga_r]
set_property PACKAGE_PIN G7 [get_ports vga_g]
set_property PACKAGE_PIN G5 [get_ports vga_b]
set_property PACKAGE_PIN D5 [get_ports vga_hsync]

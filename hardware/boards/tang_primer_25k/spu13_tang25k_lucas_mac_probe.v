...
// runs a hardcoded zero-drift test, and outputs PASS/FAIL via UART + LED.
//
...
module spu13_tang25k_lucas_mac_probe (
    ...
    spu13_lucas_mac #(.L_P(L_P), .L_P_BITS(L_P_BITS)) u_mac (
    ...
    localparam [3:0] S_PMUL    = 3;  // PMUL sanity: (3+5φ)(2+7φ) = (41+66φ)
    localparam [3:0] S_PINV    = 4;  // PINV sanity: (3+5φ)⁻¹ = (513+5φ)
    localparam [3:0] S_DRIFT_LOOP = 5;  // Zero-drift marathon
    localparam [3:0] S_PASS = 6;
    localparam [3:0] S_FAIL = 7;
    ...
    always @(posedge sys_clk) begin
        if (!rst_n) begin
            ...
            case (test_state)
                ...
                S_PMUL: begin  // (3+5φ)(2+7φ) = (41+66φ)
                    if (test_substate == 0) begin
                        mac_a <= 3; mac_b <= 5; mac_c <= 2; mac_d <= 7;
                        mac_opcode <= 2; mac_start <= 1;
                        test_substate <= 1;
                    end else if (mac_done) begin
                        if (mac_res_a == 41 && mac_res_b == 66)
                            test_state <= S_PINV;
                        else test_state <= S_FAIL;
                        test_substate <= 0;
                    end
                ...
                S_PINV: begin  // (3+5φ)⁻¹ = (513+5φ)
                    if (test_substate == 0) begin
                        mac_a <= 3; mac_b <= 5; mac_opcode <= 3; mac_start <= 1;
                        test_substate <= 1;
                    end else if (mac_done) begin
                        if (mac_res_a == 513 && mac_res_b == 5)
    ...
    always @(posedge sys_clk) begin
        if (!rst_n) begin
            ...
            start_cnt <= 0; line_timer <= 0; msg_idx <= 0;
        end else begin
            if (tx_busy) begin
                ...
                line_timer <= 0;
                case (msg_idx)
                    0: if (test_state < S_PASS) begin
                        tx_byte <= "L"; tx_go <= 1;
                    end
                    1: begin tx_byte <= "U"; tx_go <= 1; end
                    2: begin tx_byte <= "C"; tx_go <= 1; end
                    3: begin tx_byte <= "A"; tx_go <= 1; end
                    4: begin tx_byte <= "S"; tx_go <= 1; end
                    5: begin tx_byte <= ":"; tx_go <= 1; end
                    6: begin
                        tx_byte <= (test_state == S_PASS) ? "P" :
                                   (test_state == S_FAIL) ? "F" : ".";
                        tx_go <= 1;
                    end
                    7: begin tx_byte <= 8'h0D; tx_go <= 1; end
                endcase
                if (msg_idx < 7) msg_idx <= msg_idx + 1;
            end
        end
    end

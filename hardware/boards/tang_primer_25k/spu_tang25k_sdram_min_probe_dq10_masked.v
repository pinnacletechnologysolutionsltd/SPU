// Minimal Tang Primer 25K SDRAM read/write probe — DQ[10] stuck-high masked.
//
// Same as spu_tang25k_sdram_min_probe.v, but masks bit 10 in comparison and
// error masks. DQ[10] is known stuck-high (open trace between FPGA ball B2
// and SDRAM header pin 13 on this specific board). The probe ignores it to
// verify the remaining 15 data bits work correctly.

`include "spu_arch_defines.vh"

module spu_tang25k_sdram_min_probe_dq10_masked #(
    ...
    wire clk_sdram_raw = clk_div[2]; // 6.25 MHz
    wire clk_sdram;
    ...
    // ─── DQ[1] and DQ[10] are stuck HIGH on this board — mask them ───
    localparam [15:0] STUCK_MASK = 16'h0402;  // bits 1 and 10 forced to 1
    reg        match_calc;
    ...
    always @(*) begin
        ...
        for (check_i = 0; check_i < 52; check_i = check_i + 1) begin
            ...
            // Mask stuck bits out of both expected and observed before comparing.
            // DQ[1] and DQ[10] are stuck HIGH — treat as always-1 on both sides.
            exp0_obs1_mask_calc = exp0_obs1_mask_calc
                                 | ((~test_pattern(check_i[5:0]) & ~STUCK_MASK)
                                    & (rd_manifold[check_i * 16 +: 16] & ~STUCK_MASK));
            exp1_obs0_mask_calc = exp1_obs0_mask_calc
                                 | ((test_pattern(check_i[5:0]) & ~STUCK_MASK)
                                    & (~rd_manifold[check_i * 16 +: 16] & ~STUCK_MASK));
            // Compare with stuck bits forced to 1 in both expected and actual
            if ((rd_manifold[check_i * 16 +: 16] | STUCK_MASK) !=
                (test_pattern(check_i[5:0]) | STUCK_MASK)) begin
                match_calc = 1'b0;

...
module spu_dual_raster (
    ...
    // Named port connections (2026-08-24 fix): the previous positional form
    // passed three packed 64-bit {a,b,c} concatenations where
    // spu_raster_unit expects nine individual scalar ports -- iverilog
    // silently truncated/padded across the mismatch (30+ width warnings),
    // corrupting every edge coefficient past the first. Named connections
    // make the port-count/width mismatch a compile error instead of a
    // silent corruption if it recurs.
    spu_raster_unit u0 (
        .clk(clk), .rst_n(rst_n), .setup(setup0),
    ...
    spu_raster_unit u1 (
        .clk(clk), .rst_n(rst_n), .setup(setup1),

// spu_flash_bridge.v (v3.1 — W25Q128JVSQ - ECP5 USRMCLK Support)
...
`endif
    ...
    wire flash_sclk_internal;
`ifdef ARCH_ECP5
    // The USRMCLK primitive allows user logic to drive the 
    // dedicated MCLK pin after configuration is complete.
    USRMCLK u_usrmclk (
        .USRMCLKI(flash_sclk_internal),
        .USRMCLKTS(flash_cs_n)          // Tristate control (Active High)
    );
`else
    always @(*) flash_sclk = flash_sclk_internal;
`endif
    ...
    assign flash_sclk_internal = sclk_reg;

// spu_mem_bridge_sdram_x8.v
// Sovereign Manifold Bus <-> SDR-SDRAM lower-byte-only bridge
...
// Burst  : 52 lower-byte words in the 832-bit manifold
...
//
// Note: intended for bringup when the upper byte lane is suspect. The bridge
//       only drives/samples DQ[7:0]; the top must hold DQMH high.
//
// CC0 1.0 Universal.

`include "spu_arch_defines.vh"

module spu_mem_bridge_sdram_x8 #(
    ...
    reg        dq_en;
    reg [7:0] dq_out;
    assign sdram_dq[7:0]  = dq_en ? dq_out : 8'hzz;
    assign sdram_dq[15:8] = 8'hzz;
    ...
    // The board top holds DQML enabled and DQMH masked.
            ...
            case (state)
                ...
                S_BURST_RD: begin
                    ...
                    if (wr_ptr < BURST_LEN) begin
                        mem_rd_manifold[wr_ptr * 16 +: 16] <= {8'h00, sdram_dq[7:0]};
                    end
                ...
                S_BURST_WR: begin
                    if (rd_ptr < BURST_LEN) begin
                        ...
                        sdram_ba   <= a_bank;
                        sdram_addr <= {{(13-COL_BITS){1'b0}}, a_col + {{(COL_BITS-6){1'b0}}, rd_ptr}};
                        dq_out     <= mem_wr_manifold[rd_ptr * 16 +: 8];
                        dq_en      <= 1;
                        rd_ptr     <= rd_ptr + 1;

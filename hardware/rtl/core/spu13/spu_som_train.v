...
//
// Total: BMU scan (~56 cycles) + 3 cycles update
//
// Parameters:
...
module spu_som_train #(
    ...
    localparam S_DELTA  = 2;   // compute delta = feature − weight (combinational)
    localparam S_SHIFT  = 3;   // arithmetic right-shift by shift_amount
    localparam S_WRITE  = 4;   // write updated weight back
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                S_IDLE: begin
                    if (train_start && bmu_valid) begin
                ...
                S_READ: begin
                    // BRAM read issued in S_IDLE, data available next cycle
                    // Unpack rdata into w_a/w_b arrays
                    for (i = 0; i < NUM_FEATURES; i = i + 1) begin
                ...
                S_DELTA: begin
                    // Delta is combinational; register the shifted update
                    for (i = 0; i < NUM_FEATURES; i = i + 1) begin

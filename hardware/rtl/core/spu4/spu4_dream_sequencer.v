...
module spu4_dream_sequencer (
    ...
    // Sentinel Mode (Industrial Logic)
    input  wire        piranha_pulse, // 61.44 kHz Heartbeat
    input  wire        sentinel_mode, // 1 = Enable cyclic scan
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_DREAM: begin
                    ...
                    // PLC Sentinel Mode: Reset PC on piranha pulse for deterministic scan
                    if (sentinel_mode && piranha_pulse && pc != 4'd0) begin
                        pc <= 4'h0;
                        alu_start <= 1'b0;
                    end else if (current_instr[15:12] == 4'h2 && !alu_start && !alu_done) begin
                        alu_start <= 1'b1;
                    ...
                    end else if (current_instr[15:12] != 4'h2 && current_instr != 16'h0) begin
                        pc <= pc + 4'h1;
                    end
                end
            endcase

...
module spu_resonance_gen #(
    ...
    assign uv_x = $signed(cur_x) - $signed(RES_X/2);
    wire signed [15:0] uv_y;
    assign uv_y = $signed(cur_y) - $signed(RES_Y/2);

    // 2. Deformation Oscillators (Cordic-Lite)
    // Dynamic modulation from Bio-Chords
    wire [5:0] mod_speed;
    assign mod_speed = ctrl_chord[11:6];
    wire [5:0] radius_add;
    assign radius_add = ctrl_chord[5:0];
    
    // speed_tick = tick * mod_speed (scaled for stability)
    wire [15:0] speed_tick;
    assign speed_tick = (tick * mod_speed) >> 3;
    
    wire [7:0] phase_a;
    assign phase_a = (cur_x[7:0] << 2) + speed_tick[7:0];
    wire [7:0] phase_b;
    assign phase_b = (cur_y[7:0] << 2) - (speed_tick[7:0] + (speed_tick[7:0] >> 1));
    ...
    wire signed [15:0] def_x;
    assign def_x = uv_x + (sin_a >>> 6);
    wire signed [15:0] def_y;
    assign def_y = uv_y + (sin_b >>> 6);
    
    wire [31:0] dist_sq;
    assign dist_sq = (def_x * def_x) + (def_y * def_y);

    // 4. Golden Prime Dither
    // linear_id = y * width + x
    wire [31:0] linear_id;
    assign linear_id = (cur_y * RES_X) + cur_x;
    wire [15:0] golden_prime;
    assign golden_prime = 16'd65521;
    wire [31:0] noise_raw;
    assign noise_raw = linear_id * golden_prime;
    ...
    wire [31:0] dynamic_radius_sq;
    assign dynamic_radius_sq = 1600 + (radius_add << 6);

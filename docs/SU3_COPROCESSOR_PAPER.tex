...
J11 header, including all nine dense-matrix result elements on both the
standalone SU3 image and the shared-multiplier SU3SHARE image.
\end{abstract}
...
exact QR commits for all nine result elements, on the SU3SHARE image and on
the standalone sidecar spin alike (Section~\ref{sec:reproof}); the SU3SHARE
image also preserves the RPLU2 config/QR regression on the same bitstream. An
earlier 2026-07-04 capture on the standalone spin covered only three selected
elements; the 2026-08-07 re-proof supersedes it.
...
tower~\cite{rplu2026} inverts units in 83 cycles on the measured parallel v1
backend and traps
...
(Table~\ref{tab:phases}, Fig.~\ref{fig:tdm}).
...
\begin{tikzpicture}[
  font=\footnotesize,
  box/.style={draw, rounded corners=1pt, minimum height=5mm, inner sep=2.5pt, align=center},
  op/.style={box, fill=gray!8},
  acc/.style={box, fill=gray!18},
  >={Stealth[length=4pt]},
]
% operands
\node[op] (a) at (0,1.5) {$A_{31}[i]$ operand $A$\\{\scriptsize 4 imag $\|$ 4 real}};
\node[op] (b) at (0,0.35) {$A_{31}[i]$ operand $B$\\{\scriptsize 4 imag $\|$ 4 real}};

% phase selector
\node[box, fill=gray!25, minimum height=13mm] (sel) at (2.35,0.925)
  {phase\\sel.\\{\scriptsize 0..3}};

% shared multiplier
\node[box, fill=gray!12, minimum height=15mm, text width=21mm] (mul) at (4.75,0.925)
  {\texttt{spu13\_m31\_}\\\texttt{multiplier}\\[1pt]
   {\scriptsize 2-stage pipelined}\\
   {\scriptsize 16 logical products}\\
   {\scriptsize 64 DSP48E1}};

\draw[->] (a.east) -- ([yshift=3.5mm]sel.west);
\draw[->] (b.east) -- ([yshift=-3.5mm]sel.west);
\draw[->] (sel.east) -- node[above,inner sep=1pt]{\scriptsize 2 cyc/phase} (mul.west);

% partials
\node[op] (rr) at (7.55,1.85) {$RR$};
\node[op] (ii) at (7.55,1.15) {$II$};
\node[op] (ri) at (7.55,0.55) {$RI$};
\node[op] (ir) at (7.55,-0.15) {$IR$};
\foreach \n in {rr,ii,ri,ir} { \draw[->] (mul.east) -- (\n.west); }

% accumulators
\node[acc, text width=27mm] (cr) at (10.9,1.5) {$c_{\mathrm{real}} \mathrel{+}= RR - II$};
\node[acc, text width=27mm] (ci) at (10.9,0.2) {$c_{\mathrm{imag}} \mathrel{+}= RI + IR$};
\draw[->] (rr.east) -- (cr.west);
\draw[->] (ii.east) -- (cr.west);
\draw[->] (ri.east) -- (ci.west);
\draw[->] (ir.east) -- (ci.west);

% feedback
\draw[->, dashed] (cr.north) -- ++(0,0.45) -| node[pos=0.25,above,inner sep=1pt]
  {\scriptsize next $k$-term} (sel.north);

\node[align=center, font=\scriptsize] at (5.45,-0.95)
  {One complex $A_{31}[i]$ multiply = 4 phases $\times$ 2 cycles = \textbf{8 cycles}.\\
   The multiplier issues a new $A_{31}$ product every 2 cycles while the previous
   result accumulates.};
\end{tikzpicture}
\caption{Four-phase time-division multiplexing. A single $A_{31}$ multiplier
instance is time-shared across the four real/imaginary cross-products of one
complex multiply; no second multiplier is instantiated, which is what keeps the
DSP48E1 count at 64 rather than 256.}
\label{fig:tdm}
\end{figure*}
...
approximately 224 compute cycles (Fig.~\ref{fig:fsm}).
...
\begin{tikzpicture}[
  font=\footnotesize,
  >={Stealth[length=4pt]},
  cell/.style={draw, minimum height=5.5mm, minimum width=4.6mm, inner sep=0pt, font=\scriptsize},
  k0/.style={cell, fill=gray!6},
  k1/.style={cell, fill=gray!14},
  k2/.style={cell, fill=gray!24},
]
% nine result elements, each three k-terms
\foreach \idx/\lbl in {0/{$C_{00}$},1/{$C_{01}$},2/{$C_{02}$},3/{$C_{10}$},4/{$C_{11}$},
                       5/{$C_{12}$},6/{$C_{20}$},7/{$C_{21}$},8/{$C_{22}$}} {
  \pgfmathsetmacro{\xo}{\idx*1.70}
  \node[k0] (a\idx) at (\xo,0)      {$k_0$};
  \node[k1] (b\idx) at (\xo+0.48,0) {$k_1$};
  \node[k2] (c\idx) at (\xo+0.96,0) {$k_2$};
  \node[font=\scriptsize] at (\xo+0.48,0.55) {\lbl};
  % emit pulse
  \draw[->] (c\idx.east) -- ++(0.30,0) node[right, inner sep=1pt, font=\scriptsize] {};
  \node[font=\scriptsize, anchor=west] at (\xo+1.30,0) {\raisebox{0pt}{$\downarrow$}};
}
% brace for one element
\draw[decorate, decoration={brace, amplitude=3pt, mirror}]
  (-0.24,-0.42) -- (1.20,-0.42)
  node[midway, below, inner sep=3pt, font=\scriptsize]
  {3 complex multiplies $\times$ 8 cyc = \textbf{24 cyc}};

% total
\draw[decorate, decoration={brace, amplitude=4pt, mirror}]
  (-0.24,-1.28) -- (14.36,-1.28)
  node[midway, below, inner sep=4pt, font=\scriptsize]
  {9 elements $\times$ 24 cyc = \textbf{216 compute cycles};
   with load/commit overhead, \textbf{$\approx$224 cycles} end to end};

\end{tikzpicture}
\caption{Row-major schedule of the $3\times3$ multiply. Each result element
accumulates its three $k$-terms and is emitted ($\downarrow$) as soon as it
completes, so the
result stream is part of the compute schedule rather than a separate drain
stage. The $\approx$224-cycle latency is 216 cycles of arithmetic plus load and
commit overhead --- there is no reorder buffer and no output queue.}
\label{fig:fsm}
\end{figure*}
...
and a 40-second capture completed thirteen three-case passes.
...
\subsection{Re-proof and Correction (2026-08-07)}
\label{sec:reproof}

The 2026-07-04 run above was never entered in the project's hardware
evidence ledger, and its exact conditions are not reproducible from
committed artifacts: the smoke firmware's SPI rate and its four guard
delays are compile-time defaults of 25\,kHz and 1000\,$\mu$s, with no
build-time override anywhere in the repository. The 100\,kHz /
20\,$\mu$s figures therefore describe an uncommitted local build. The run
was repeated to establish what is reproducible; it is logged in full as
\S3.2e.6 of the project hardware evidence ledger.

On the unchanged bitstream (SHA-256 \texttt{a8b9f661892fd052}\ldots), the
standalone spin returned all \emph{nine} dense-product elements exactly, on
identity lane mapping, across twelve consecutive complete runs with no failed
element and no invalid QR read. The three elements in
Table~\ref{tab:silicon} reproduce character for character, confirming those
values on silicon rather than by transcription --- a distinction that matters
here, since the same constants are compiled into both the smoke firmware and
the Python oracle and were available without running anything. The lane
assignments in Table~\ref{tab:silicon} were correct for the 2026-07-04
firmware, which mapped three selected elements onto lanes 2, 5 and 8; the
current firmware returns all nine on identity mapping.

A guard-delay sweep on the same bitstream, rebuilding the firmware for each
configuration, gave 12/12 passes at 25\,kHz with 1000\,$\mu$s guards, 27/27 at
100\,kHz with 20\,$\mu$s, and 44/44 at 100\,kHz with 5\,$\mu$s: 83 complete
runs with no failed element and no invalid QR read at any setting.
...
\begin{itemize}
\item \textbf{Guarded bring-up link:} the silicon proof runs over a guarded
SPI stream, with committed firmware defaults of 25\,kHz and 1000\,$\mu$s guard
delays. An earlier 5\,$\mu$s probe on the 2026-07-04 pre-reset-fix bitstream
reported intermittent QR read invalidity; that observation did \emph{not}
reproduce on 2026-08-07, where 44 consecutive complete runs at 100\,kHz with
5\,$\mu$s guards returned no invalid read. It was most likely resolved by the
reset conditioning subsequently applied to every Artix constraint file, which
removed an unconditioned \texttt{rst\_n} pad --- a credible source of
intermittent reads at tight guard timings. Confirming that would require
reloading a pre-fix bitstream, which has not been done. The link therefore has
no characterised minimum guard interval, and no margin figure should be quoted
...
phase coherence on FPGA,'' 2026. DOI:
\href{https://doi.org/10.5281/zenodo.21447440}{10.5281/zenodo.21447440}.

\bibitem{rplu2026}
J.~Curley, ``A hardware jet algebra coprocessor over a split M31
biquadratic algebra,'' 2026. DOI:
\href{https://doi.org/10.5281/zenodo.21446712}{10.5281/zenodo.21446712}.

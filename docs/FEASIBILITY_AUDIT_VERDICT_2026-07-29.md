...
| Locality-aware FPGA floorplanning | `HOLD` | Run one selective-hierarchy synthesis measurement after the FP4 default-switch work closes; no P&R campaign. |
...
The cyclotomic oracle is now closed. The remaining priority is **the bounded
floorplanning synthesis measurement when its existing scheduling gate clears,
then no tetrahedral implementation**.
...
- The only authorized next measurement is selective `keep_hierarchy` on 3-5
  modules. Gate: at least 90% target-cell prefix coverage and no more than 2%
  total cell-count growth. Failure moves the physical experiment to `NO_GO`.
...
3. Declared length-100 growth corpus: **complete**, 55 signed coefficient bits
   maximum on the measured corpus.
...
7. After the existing scheduling gate clears, run only the bounded
   selective-hierarchy floorplanning measurement.

The tetrahedral and Chaitin candidates remain documentation-only.

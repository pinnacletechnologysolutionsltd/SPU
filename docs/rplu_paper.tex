...
unrolled), with bit-identical replay across simulation runs. A conjugate
reduction tower inverts unit elements in
$\sim$76 deterministic cycles, and a lookahead hazard unit (LHU) schedules
concurrent single-cycle arithmetic alongside tower operations.
...
a Xilinx Artix-7 100T implementation (240 DSP48E1, $\sim$100\,K LUTs)
using DSP cascade routing for zero-routing-overhead systolic multipliers,
and a Gowin GW5A-25A prototype (8,256 LUT4s) demonstrating subsystem
feasibility at 67\% device utilization. The architecture eliminates
...
The BTU (Bus Transmutation Unit) is a Direct Combinational Demultiplexer
...
\subsection{The Execution Layer: LHU Scoreboarding}

The 4R2W multi-port register file (\texttt{spu13\_multi\_port\_regfile.v})
supports concurrent single-cycle integer arithmetic (QADD, QSUB, QMUL)
alongside the 76-cycle Conjugate Inversion Tower. The Lookahead Hazard
Unit (LHU) tracks register scoreboard bits for RAW and WAW hazards.

\textbf{Key design choice:} Single-cycle ops may issue every cycle without
stalling---they write through a write-forwarding bypass path. Tower ops
(PHSLK, INVJ, Pad\'{e} eval) stall issue until the tower completes and
writes back. Since the tower latency is fixed and the scoreboard depth
is 2 (one tower + one single-cycle write), all hazard resolution is
combinational---no dynamic arbitration or reorder buffers required.
...
~88-cycle window. The LHU successfully manages RAW hazards when the Pad\'{e}
output is immediately consumed by a downstream QADD---the write-forwarding
path delivers the result on the same cycle as writeback.
...
(primary target, 240 DSP48E1, $\sim$100\,K LUTs), the full pipeline
consumes $\sim$1,172 LUTs and 16 DSP slices---8\% of DSP capacity and
9\% of LUTs, leaving substantial headroom for the fully unrolled jet MAC
(96 DSPs) or additional SOM nodes. On the \textbf{Gowin GW5A-25A}
...
but motivating the Artix-7 as the target for full concurrent operation.
...
board, 240 DSP48E1, $\sim$100\,K LUTs) as the primary target for full
...
K_{k-i}$ for $k \in \{0,1,2\}$, verified against 28 test vectors across
...
serial dispatch,
\item A Lookahead Hazard Unit enabling concurrent single-cycle arithmetic
alongside multi-cycle tower operations,
...
implementation target and a Gowin GW5A-25A prototype path. All RTL, software
oracles, and documentation are open-source (CC0 1.0).

\section*{Acknowledgment}
All contributions are CC0 1.0 Universal (public domain).
Open-source at: \url{github.com/spu13}.

...
\href{mailto:johncurley@pinnacletechglobal.com}{johncurley@pinnacletechglobal.com}}
}

\maketitle

\noindent\fbox{\parbox{0.97\columnwidth}{\footnotesize
\textbf{Version 0.2 corrigendum (2026-07-26).} Version 0.1
(DOI 10.5281/zenodo.21446713) is superseded for the following reason:
\textbf{Corrected inverter latency and Montgomery batch-inversion cycle
model.} Version 0.1
understated the historical parallel
conjugate-tower latency as 76 clocks and propagated that constant into the
Montgomery batch-inversion model. Measurement against the deposited
\texttt{f1e4dbf} RTL gives 83 clocks for unit completion and 7 clocks for
Stage-B singular termination; the complete jet inverse takes 105 clocks for
units. Table~\ref{tab:corrigendum-delta} gives the old-versus-corrected
values; Table~\ref{tab:batch-inv} and all dependent latency statements are
regenerated from those measured constants. Affected locations are the
abstract; Jet Inverse (Shadow Chain); Stage $\Phi_3$---The Conjugate
Reduction Tower; Montgomery Batch Inversion Speedup; Scoreboarded Tower
Overlap; and the conclusion. Arithmetic results, routed resources, and
silicon evidence are unchanged. The measurement protocol and raw output are
preserved in
\texttt{docs/RPLU\_V01\_LATENCY\_CORRIGENDUM\_EVIDENCE.md}.}}
\vspace{0.5em}
...
silicon-tested image. The measured historical parallel conjugate tower
inverts unit elements in 83 clocks; the complete jet inverse, including
wrapper control and the six-multiply shadow chain, takes 105 clocks. Issue
control is deterministic and blocking:
...
\vspace{0.5em}
\noindent\fbox{\parbox{0.97\columnwidth}{\footnotesize
\textbf{Reader's map.} This paper describes one arithmetic engine of
the SPU-13, a deterministic FPGA coprocessor family; understanding the
complete SPU-13 instruction set or its Quadray coordinate system is
not required here. A Pad\'{e} approximant is a ratio of two
polynomials used to approximate a nonlinear function; evaluating one
needs only multiplication, addition, and one division-like inversion,
which is why it suits hardware with no floating point. The
contribution of this paper is the exact evaluation of such rational
functions --- and the inversion they require --- over a finite
algebraic ring on FPGA, with every table regenerable from a committed
software oracle.}}
\vspace{0.5em}
...
where $j_0^{-1}$ is computed by the 83-cycle parallel conjugate reduction tower
(Section~\ref{sec:tower}) and $m_0, m_1, m_2$ are the inverse jet
coefficients. Wrapper control and the six-multiply shadow chain add 22 clocks
after the tower's accepted-start-to-done interval, for a measured total unit
latency of 105 clocks.
...
Measured latency on the historical uncontended parallel backend is 83 clocks
for unit completion and 7 clocks for Stage-B singular termination, with no
operand-dependent variance inside either outcome class. There are zero
division operations. Inversion test vectors are shown in
Table~\ref{tab:fp4-inverter}.
...
tower cost of $k \cdot 83$ cycles is dominated by the serial Fermat-chain
...
breaks even is $k = 2$; the corrected asymptote is $3.06\times$ speedup with a
...
83-cycle parallel Conjugate Inversion Tower.
...
batch inversion vs.\ per-element tower runs, using the measured historical
costs of 83 cycles per unit tower and 3 cycles per shared multiply. The modeled
...
manifold sweep), the batch path saves $62.1\%$ of the per-element tower
...
RPLU2PADE image. The table is generated by
\texttt{tools/rplu\_paper\_data.py} from the same constants used by the
executable batch-inversion oracle.
...
105-cycle unit-inversion window is idle time for the rest of the pipeline.
...
\item An 83-cycle parallel Conjugate Reduction Tower with a 7-cycle
Stage-B singular path, enabling a 105-cycle complete unit jet inverse,

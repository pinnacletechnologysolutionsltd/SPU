# Tensegrity Balancer Handover — 2026-07-15/16/18/19

## Closure update — 2026-07-19

The complete `TENSEGRITYLINK` transaction is now silicon-proven on the Wukong
Artix-7.  The parser-bounded term-serial image admitted the canonical balanced
table, committed the genuine not-in-equilibrium fixture, rejected a corrupted
TGR1 payload while preserving that active fault verdict, and recovered on a
canonical reload.  The sequence passed three consecutive times through the
real SD card, RP2350 firmware, remapped J11 link, B2 transport, combined
intersection+equilibrium guard, and B3 readback.

The packed bitstream uses 25,563 SLICE_LUTX, 8,980 SLICE_FFX, 66 DSP48E1, and
one RAMB18E1; guard Fmax is 41.54 MHz at the 25 MHz cadence.  SHA-256:
`30381825ed444d92a5474740c0219c84fff449e05ba575d45dcbb409459a1de5`.
The payload-negative was generated by the bench-only `tgrloadbadcrc` command,
which retained a valid transport CRC-8 while corrupting one payload byte; FPGA
loader error 7 therefore proves the independent TGR1 CRC-32 rollback boundary.

Parser replay is now observable as substages `0x11` through `0x1A` and bounded
by an independent watchdog.  A forced parser wait-state stall returns error 11
and `0x90 | substate`; regression pins `0x93`, rollback, and recovery.  This
instrumentation also perturbed placement, so it did not isolate the precise
logical cause of the old stage-1 stall.  The silicon claim is tied to the hash
above.  The remaining frontier is the active proposal/actuation controller.

## Historical stopping point through 2026-07-18
...
- First-hour tensegrity demo: 17 checks, including explicit stuck-stage
  reporting.
...
The parser regression separately forces replay substate 3 to stall, proves
parser error 11 and stage `0x93`, preserves the active transaction, and then
proves recovery.
...
At that historical checkpoint, the canonical combined verdict was still
pending because the attached
...
silicon-proven. The 2026-07-19 closure above supersedes this restriction.
...
The combined diagnostic/build tranche is closed. Product work can return to
the already-proven rational SOM classifier/sidecar; further tensegrity work
should begin with an explicit active-control contract, not more admission
diagnostics.

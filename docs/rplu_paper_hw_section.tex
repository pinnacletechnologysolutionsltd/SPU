...
| Status | Wukong JTAG/J11 silicon proofs | Split-probe silicon regression |
| Role | Silicon evidence and constrained integration | Subsystem proof, probe ladder |
...
Each term $J_i \cdot K_{k-i}$ is a full $A_{31}$ multiplication requiring 16
...
The XC7A100T provides 240 DSP48E1 and 63,400 LUT6-class logic cells. The table
above is a design budget for the local arithmetic lane, not a routed whole-image
claim. Current routed Artix evidence should be taken from
`docs/hardware_evidence.md`: the `RPLU2PADE` Wukong proof uses 72 DSP48E1 cells
and passes the 2 MHz bring-up target, while `SU3SHARE` is the current shared
multiplier integration baseline. Whole-image builds on the 100T have tight
placement margin once live RPLU2, sidecars, and safety layers coexist. The fully
concurrent integration target is therefore the 200T / Kintex-class path; the
100T remains the silicon-evidence and constrained-integration target.

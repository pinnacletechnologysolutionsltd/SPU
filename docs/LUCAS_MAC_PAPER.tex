...
\begin{itemize}
    ...
    representation is expressible over $\mathbb{Z}[\phi]/L_p$; it is not.
    \textbf{Future work:} a degree-4 cyclotomic MAC over
    $\mathbb{Z}[\zeta_5]/L_p$, which would carry the $R$-matrix phases
    natively and subsume the present ring as its real subfield. The datapath
    generalization is structural rather than novel --- $\mathbb{Z}[\zeta_5]$ is
    a rank-4 module where $\mathbb{Z}[\phi]$ is rank-2 --- but the reduction,
    zero-divisor, and period analyses of the Mathematical Foundation section
    would all need redoing for the larger ring.
\end{itemize}

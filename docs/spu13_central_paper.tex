...
The SOM/BMU group (Section~\ref{sec:som}) seeds the RPLU pipeline's spatial
classifier.
...
\item[$\Phi_3$] \textbf{Thimble-Pad\'e} -- $[4/4]$ rational Pad\'e approximant
  ...
  $\mathbb{A}_{31}$ denominator inversion (parallel v1: 83-cycle unit /
  7-cycle singular, deterministic within outcome class).
...
\subsection{SOM/BMU Front-End}
\label{sec:som}

The SOM/BMU stage performs deterministic spatial classification that seeds
the BTU route. Given a set of $N$ weight vectors
$w_j \in \mathbb{Q}(\sqrt{3})^3$ and an input
$x \in \mathbb{Q}(\sqrt{3})^3$, the best-matching unit is the weight
minimizing the weighted quadrance:

\begin{equation}
Q(x, w_j) = \sum_{i=1}^{3} \alpha_i (x_i - w_j)^2,\quad
\alpha_i \in \mathbb{Q}
\label{eq:weighted_quadrance}
\end{equation}

where subtraction, squaring, and accumulation are all exact in
$\mathbb{Q}(\sqrt{3})$. The winner-take-all tree resolves ties by a
deterministic secondary comparison on the quadratic coefficients, ensuring
bit-exact replay across Python, C++, and RTL implementations.

Node weights are stored in four parallel block-RAM slices (one per feature
dimension), each parameterized up to 64 entries, and are hydrated at boot from
the SPI host or on-chip flash. An input-range gatekeeper assertion
($|P|,|Q| < 2^{15}$) prevents the 18-bit subtraction truncation edge case
identified during formal audit, and a dyadic training engine
($w \leftarrow w + (x-w) \gg k$) provides on-chip learnability with a
documented 1~LSB/step rounding divergence from the exact-rational Python
oracle.

Verification proceeds in three stages.  Stage~1 (software oracle): a Python
reference model and C++17 parity implementation covering 24 scenarios
(integer and surd quadrances, field-square reduction, stable tie-breaking).
Stage~2 (silicon probe): the Tang~25K SOM/BMU probe validates the full
classify path (7-node hex fixture, 4 material classes) via on-chip UART
telemetry with 112/112 regression passes across Verilog, C++, and Python.
Stage~3 (dataset baseline): the same 64-node hex-grid fixture achieves
95.33\% accuracy on the Fisher Iris dataset (150 samples, 3 classes, 4
features) --- within 2\% of floating-point $k$-nearest neighbours, with full
deterministic replay across all implementation layers.

A separate hydration probe (583~LUT, 165~FF, 8~BRAM) verifies BRAM
write/readback and per-feature byte-enable hydration, confirming the SPI
$0\times$A5 $\tt sel=4$ path for runtime weight updates without
re-synthesis.
...
\midrule
Tang 25K: som\_bmu          & 15,325 & 1,009 & 0 & 4 \\
Tang 25K: som\_hydrate      & 583 & 165 & 0 & 8 \\
...
\begin{itemize}
\item SOM/BMU: 112/112 regression passes across Verilog, C++, and Python;
  Iris accuracy 95.33\% on 64-node hex fixture.

...
| Bounded cycle contract | **RTL — measured, simulation only** | Bounded, **not fixed**: 180–183 clocks measured over 124 operations (4 corners + 120 randomised), contract bound **200 clocks**, asserted every run by `spu4_customer_wrapper_tb`. Latency varies with operand values because the multiplier is serial. No board has run the wrapper. `docs/SPU4_ABI.md` §4. |
